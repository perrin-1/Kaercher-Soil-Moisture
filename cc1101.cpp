...
// new PATABLE RX 0506 0812
const byte paTable[8] = {0x00, 0x03, 0x0F, 0x25, 0x67, 0x40, 0x60, 0x50};
...
byte CC1101::receiveData(CCPACKET * packet)
{
  ...
  else if (readStatusReg(CC1101_RXBYTES) & 0x7F)
  {
    ...
    //subtract status bytes (RSSI, LQI, CRC)
    packet->length = rxBytes - 3;
    //Serial.print(" Num bytes RXFIFO ");
    ...
    else
    {
      ...
      // Read LQI and CRC_OK
      val = readStatusReg(CC1101_LQI);
      packet->lqi = val & 0x7F;
      packet->lqi = 0x3F - (packet->lqi & 0x3F);
      packet->crc_ok = bitRead(val, 7);
      //Serial.println("IDLE+FLUSH");
      //cc1101_Deselect();
      //cc1101_Select();
      setIdleState();       // Enter IDLE state
  ...
  //not needed as stay in RX is on => MCSIM1 = 0C
  //setRxState();
